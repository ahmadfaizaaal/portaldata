<main>
				...
				<div class="row">
					...
					<div class="col-xl-12">
						<div class="hero-cap2 pt-20 text-center">
							...
							<a href="#" id="download-full" class="btn text-white radius btn-lg bg-pupr-yellow px-5 py-3 mt-5 f-16 download-btn disabled" data-file-name="Rekapitulasi-TTK-SKTK-Masa-Transisi" data-sheet-name="Rekapitulasi TKK.Rekapitulasi Tenaga Terampil.Provinsi Domisili TTK.Kabupaten Kota Domisili TTK" data-source="resData" data-order-index="skk_jumlah_tkk.skt_jumlah_tkk.provinsi_domisili_tkk.kabkot_domisili_tkk" disabled><span><i class="fa fa-solid fa-download"></i></span>&nbsp;&nbsp;Download Full Data Tenaga Terampil (SKTK Masa Transisi)</a>
						</div>
					</div>
		...
		<div class="container">
			...
			<div class="row" id="kabupaten-kota-section">
				<div class="col-md-12 text-center my-5 py-3">
					...
					<a id="download-kabkot" href="#kabupaten-kota-section" class="btn radius btn-lg bg-pupr-blue text-white download-btn disabled" data-file-name="Kabupaten-Kota-Domisili-Tenaga-Terampil" data-sheet-name="Kabupaten Kota Domisili" data-source="resData.kabkot_domisili_tkk" data-order-index="kabkot_domisili_tkk" disabled><span><i class="fa fa-solid fa-download"></i></span>&emsp;Download Data Kabupaten Kota Domisili Tenaga Terampil</a>
				</div>
				<div class="col-md-12">
					<table class="table table-bordered" width="100%" id="kabupaten-kota">
						<thead>
							<tr>
								...
								<th class="text-center" width="30%">Total Tenaga Terampil</th>
							</tr>
						</thead>
						<tbody>
							<tr>
	...
	<script>
		var ctx = document.getElementById("provDomisiliTTKMaps") // Get Canvas Element
		...
		$(window).on("load", function() {
			...
			$.get(baseUrl + `/${devMode ? "tenagakerja-dev" : "tenaga-kerja"}/terampil/ajax`, (res) => {
				res = $.parseJSON(res)
				if (res.status == 200) {
					...
					const ElementJumlahTKK = $("#ajax-jumlah-tkk").html((resData.skk_jumlah_tkk.jumlah_tkk.toLocaleString()))
					const ElementJumlahTTK = $("#ajax-jumlah-ttk").html((resData.skt_jumlah_tkk.jumlah_ttk.toLocaleString()))

					// Provinsi Registrasi Database Array
					const dataProvDomisili = resData.provinsi_domisili_tkk
							...
							const data = {
								...
								datasets: [{
									...
									data: indonesiaTopo.map(provinsi => ({
										feature: provinsi,
										value: (search(provinsi.properties.provinsi, dataProvDomisili) ? search(provinsi.properties.provinsi, dataProvDomisili).jumlah_ttk : 0)
					...
					const tableHeaders = {
						...
						provinsi_domisili_tkk: {
							provinsi_personal: "Provinsi Domisili Tenaga Terampil",
							jumlah_ttk: "Total Tenaga Terampil"
						},
						kabkot_domisili_tkk: {
							provinsi_personal: "Provinsi Domisili Tenaga Terampil",
							kabupaten_personal: "Kabupaten/Kota Domisili Tenaga Terampil",
							jumlah_ttk: "Total Tenaga Terampil"
						}
					}
